...
def _build_generation_objects(
    ...
    generation_config = GenerationConfig(
        **cfg.sampling_params,
        ...
        eos_token_id=_eos_token_ids(cfg),
    )
    return logits_processor, generation_config


def _eos_token_ids(cfg):
    token_ids = [cfg.special_token_ids["EOS"]]
    if getattr(cfg, "stop_on_image_end", False):
        token_ids.append(cfg.special_token_ids["EOI"])
    return token_ids
...
def build_logits_processor(
    ...
    logits_processor = UnbatchedClassifierFreeGuidanceLogitsForVisualTokenWithDifferentialTopKProcessor(
        ...
        force_same_image_size=force_same_image_size,
        max_hw_digits=getattr(cfg, "max_hw_digits", 3),
