...
class UnbatchedClassifierFreeGuidanceLogitsForVisualTokenProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        cfg_decay: str = "none",
        cfg_min: float = 1.0,
        cfg_total_images: int = 1,
    ):
        self.guidance_scale = guidance_scale
        self.cfg_decay = str(cfg_decay).lower()
        self.cfg_min = float(cfg_min)
        self.cfg_total_images = max(1, int(cfg_total_images))
        self.model = model
        ...
        self.hw_tokens = None
        self.max_hw_digits = max(1, int(max_hw_digits))
        self.digit_token_ids = [
            self.tokenizer.encode(str(digit), add_special_tokens=False)[0]
            for digit in range(10)
        ]
        self.nonzero_digit_token_ids = self.digit_token_ids[1:]
        self.star_token_id = self.tokenizer.encode("*", add_special_tokens=False)[0]
        self.force_same_image_size = force_same_image_size
        self.unconditional_type = unconditional_type

        self.text_segment = 0

        if self.force_same_image_size and IMG in unconditional_ids[0]:
    ...
    def in_image_logits_processor(self, input_ids, scores):
        ...
        if self.in_visual:
            ...
            # visual token
                ...
                scores = self.apply_cfg(
                    scores,
                    unc_scores,
                    self.current_guidance_scale(),
            ...
            # not yet at IMG token: we are generating h x w tokens
                ...
                if self.height is not None and self.width is not None:
                    ...
                    if self.first_in_image or not self.force_same_image_size:
                        allowed_tokens = self.allowed_auto_hw_tokens(input_ids)
                        mask = torch.full_like(scores, -math.inf)
                        for token in allowed_tokens:
                            mask[:, token] = 0
                        scores = scores + mask
    ...
    def allowed_auto_hw_tokens(self, input_ids):
        boi_idx = self.find_last_token_index(input_ids[0], BOI)
        hw_tokens = input_ids[0, boi_idx + 1:].tolist()

        if self.star_token_id not in hw_tokens:
            height_digits = len(hw_tokens)
            if height_digits == 0:
                return self.nonzero_digit_token_ids
            if height_digits < self.max_hw_digits:
                return self.digit_token_ids + [self.star_token_id]
            return [self.star_token_id]

        star_idx = hw_tokens.index(self.star_token_id)
        width_digits = len(hw_tokens) - star_idx - 1
        if width_digits == 0:
            return self.nonzero_digit_token_ids
        if width_digits < self.max_hw_digits:
            return self.digit_token_ids + [IMG]
        return [IMG]
    ...
    def current_guidance_scale(self):
        if self.cfg_decay in {"", "none"}:
            return self.guidance_scale

        step = max(0, self.image_nums - 1)
        if self.cfg_total_images <= 1:
            progress = 1.0
        else:
            progress = min(step, self.cfg_total_images - 1) / (self.cfg_total_images - 1)

        if self.cfg_decay == "linear":
            return self.guidance_scale + (self.cfg_min - self.guidance_scale) * progress
        if self.cfg_decay == "cosine":
            return self.cfg_min + 0.5 * (self.guidance_scale - self.cfg_min) * (
                1.0 + math.cos(math.pi * progress)
            )
        raise ValueError(f"Unsupported cfg_decay {self.cfg_decay!r}")
