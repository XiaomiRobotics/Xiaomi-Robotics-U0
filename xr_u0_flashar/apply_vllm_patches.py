...
def run_patch(patch_file: Path, site_dir: Path, *, dry_run: bool = False) -> tuple[bool, str]:
    cmd = ["patch", "-p2", "--silent"]
    if dry_run:
        cmd.append("--dry-run")
...
def cmd_apply(site_dir: Path) -> None:
    ...
    backup_root = site_dir.parent / "vllm_unis_flashar_patch_backup"
    ...
    print(f"[ok] FlashAR patch flow complete; applied {applied} patch(es)")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Apply Xiaomi-Robotics-U0-FlashAR vLLM 0.11.0 patches.")
